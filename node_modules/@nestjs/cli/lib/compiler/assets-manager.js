...
class AssetsManager {
    ...
    resolveAssetDestination(filePath, outDir, sourceRoot) {
        const dest = (0, copy_path_resolve_1.copyPathResolve)(filePath, outDir, sourceRoot.split(path_1.sep).length);
        const projectRoot = process.cwd();
        const resolvedDest = (0, path_1.resolve)(projectRoot, dest);
        const relativeDest = (0, path_1.relative)(projectRoot, resolvedDest);
        const isProjectRoot = relativeDest === '';
        const isOutsideProject = relativeDest === '..' ||
            relativeDest.startsWith(`..${path_1.sep}`) ||
            (0, path_1.isAbsolute)(relativeDest);
        if (isProjectRoot || isOutsideProject) {
            throw new Error(`Refusing to process asset outside of or equal to the project directory: ${dest}`);
        }
        return resolvedDest;
    }
}
